				...
				if (touches[0].phase == TouchPhase.ENDED) {
					...
					if(targ == goback.SourceImage){
						...
						if(ItIsLit === false){
							if(PhraseCounter == 0){
								PhraseCounter = 1;
								(stage.getChildAt(0) as Object).screenGamePlayHandler.ReadOutObj.ToggleReadOut("The whole room feels like its contorting in one great spiral.");
							...
							if(PhraseCounter == 0){
								PhraseCounter = 1;
								(stage.getChildAt(0) as Object).screenGamePlayHandler.ReadOutObj.ToggleReadOut("The walls pulse with a magentic feild");
							}else if(PhraseCounter == 1){
								PhraseCounter = 2;
